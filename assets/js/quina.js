let localquina = document.getElementById("resultado-quina");
fetch('https://loteriascaixa-api.herokuapp.com/api/quina/latest')
    ...
    .then(data => {
        console.log(data);
        document.getElementById('concurso-quina').innerHTML = data.concurso;
        document.getElementById('data-quina').innerHTML = data.data;
        for(const i in data.dezenas){
            data.dezenas[i];
            document.getElementById("dezenas-quina").innerHTML += `
        ...
        if (data.acumulou == false) {
            let venceuquina = `<div class="comganhador">
      ...
      localquina.innerHTML = `${venceuquina}`;
        } else {
            let venceuquina = `<div class="semganhador">
      ...
      localquina.innerHTML = `${venceuquina}`;
        ...
        if(data.dataProxConcurso == ""){
          document.getElementById('proximoconcurso-quina').innerHTML = `${data.proxConcurso}`;
        }else{
          document.getElementById('proximoconcurso-quina').innerHTML = `${data.proxConcurso} em ${data.dataProxConcurso}`;
        }
        document.getElementById('local-quina').innerHTML = `Local: ${data.local}`;
    })
    .catch(error => {
        console.log(error);
    });